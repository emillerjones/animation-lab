...
function PhotonMotes({ count, speed }) {
  const setup = useMemo(() => {
    ...
    system.emitter.position.set(2.8, 0.45, 0);
...
function IncidentLight({ strength }) {
  const light = useRef();
  const target = useRef();
  const source = useMemo(() => new THREE.Vector3(-9, 4.2, 4.4), []);
  const destination = useMemo(() => new THREE.Vector3(2.8, 0.45, 0), []);
  const beam = useMemo(() => {
...
function SpectralFan({ body, dispersion }) {
  const group = useRef();
  ...
  return (
    <group ref={group} position={[2.8, 0.45, -0.15]}>
...
function CrystalRig({ die, envMap, dispersion, speed, onDraggingChange }) {
  ...
  return (
    <>
      <SpectralFan body={body} dispersion={dispersion} />
      <RigidBody
        ref={body}
        key={die}
        position={[2.8, 0.45, 0]}
...
function OpticalBench() {
  return (
    <group>
      <mesh position={[2.8, -2.55, -1.5]} rotation={[-Math.PI / 2, 0, 0]}>
      ...
      {[3.6, 4.8, 6.2].map((radius, index) => (
        <mesh key={radius} position={[2.8, 0.45, -2.6]} rotation={[0, 0, index * 0.45]}>
...
function PrismScene({ settings, onDraggingChange }) {
  ...
  return (
    <>
      ...
      <IncidentLight strength={light} />
      <OpticalBench />
      <PhotonMotes count={Math.round(settings.motes ?? 44)} speed={speed} />
      {envMap && (
  ...
  return (
    <section className={`atmosphere prism-pulse-lab ${dragging ? "is-dragging" : ""}`} style={{ "--experiment-accent": "#77dcff" }}>
      ...
      <div className="experiment-copy">
        <p>07 — Refractive geometry / {die}</p>
